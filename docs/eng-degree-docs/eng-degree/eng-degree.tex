...
\begin{itemize}
    ...
    \item Jest ma olbrzymią społeczność z dużą liczbą ekspertów, którzy go używają i otwarcie dzielą się wiedzą;
    \item liczba matcherów i funkcjonalności może stać się przytłaczająca i prowadzić do mniejszej produktywności;
...
\begin{itemize}[label=--]
    ...
    \item mikroserwis emailowy -- jego jedynym zadaniem jest obsługa wysyłania wiadomośći email;
    \item bramka interfejsu aplikacji (eng. \textit{api gateway}) - służąca za punkt dostępu do interfejsu, w której odbywa się komunikacja między mikroserwisami oraz autoryzacja i autentykacja użytkowników.
\end{itemize}

\subsection{Diagramy sekwencji}

Diagramy sekwencji przedstawiają istniejące w aplikacji serwerowej procesy, umożliwiające z niej użytkowanie.

\begin{figure}[h]
    \textbf{\caption{Diagram logowania użytkownika}}
    \centering{\includegraphics[width=15cm]{./static/diagrams/diagram-logowania.png}}\\
    \raggedright\scriptsize{Źródło: Opracowanie własne}
\end{figure}

Rysunek 3.1 przedstawia diagram procesu logowania użykownika wraz z drzewami decyzyjnymi.
...
\begin{itemize}[label=--]
    ...
    \item podanie poprawnych danych i potwierdzony adres email -- zwrócenie tokenów autoryzacyjnych.
\end{itemize}

\begin{figure}[h]
    \textbf{\caption{Diagram zmiany adresu email}}
    \includegraphics[width=15cm]{./static/diagrams/diagram-zmiany-adresu-email.png}\\
    \scriptsize{Źródło: Opracowanie własne}
\end{figure}

Rysunek 3.2 przedstawia diagram zmiany adresu email.
...
\begin{figure}[h]
    ...
    \vspace{0.55em}
    \centering\includegraphics[width=15cm]{./static/diagrams/diagram-pobierania-danych-smietniczkow.png}\\
    \raggedright\scriptsize{Źródło: Opracowanie własne}
\end{figure}

Rysunek 3.3 przedstawia diagram pobierania danych śmietników.
...
\begin{figure}[h]
    ...
    \vspace{0.55em}
    \centering\includegraphics[width=15cm]{./static/diagrams/naprawiony-diagram-rejestracji.png}\\
    \raggedright\scriptsize{Źródło: Opracowanie własne}
\end{figure}

Rysunek 3.4 przedstawia diagram rejestracji użytkownika. Po wypełnieniu formularzu rejestracji użytkownik otrzymuje wiadomość email pod wskazanym adresem wraz z linkiem do potwierdzenia rejestracji. Po wejściu na witrynę do której prowadzi link, konto użytkownika jest potwierdzone i może rozpocząć on korzystanie z aplikacji.


\begin{figure}[h]
    \textbf{\caption{Diagram resetu hasła użytkownika}}
    \vspace{0.55em}
    \includegraphics[width=13cm]{./static/diagrams/diagram-resetu-hasla.png}\\
    \scriptsize{Źródło: Opracowanie własne}
\end{figure}

\subsection{Diagram widoków aplikacji}
\begin{figure}[H]
    \centering
    \textbf{\caption{Widoki aplikacji mobilnej w formie diagramu przepływu}}
    \includegraphics[width=12.5cm]{./MobileAppFlows.png}
    \raggedright{\scriptsize{\\Źródło: Opracowanie własne}}
\end{figure}
...
\begin{figure}[H]
    ...
    \vspace{0.8em}
    \includegraphics[width=13.5cm]{./static/CPU-Usage-Comparison-Flutter-vs-Native-iOS-Android.png}
...
\begin{thebibliography}{9}
    ...
    \bibitem{gRPC-performance} Mateus Araújo [i inni]. Performance analysis of computational offloading on embedded platforms using the gRPC framework. 8th International Workshop on ADVANCEs in ICT Infrastructures and Services (ADVANCE 2020), Candy E. Sansores, Universidad del Caribe, Mexico, Nazim Agoulmine, IBISC Lab, University of Evry - Paris-Saclay University, Jan 2020, Cancún, Mexico
    \bibitem{review-image-classification-CNN} Leiyu Chen, Shaobo Li, Q. Bai, Jing Yang, Sanlong Jiang, Yanming Miao, Review of Image Classification Algorithms Based on Convolutional Neural Networks, College of Mechanical Engineering, Guiyang 550025, Chiny, 21.11.2021
    \bibitem{regularization-deep-learning} Mehdi Sajjadi, Mehran Javanmardi, Tolga Tasdizen, Regularization With Stochastic Transformations and Perturbations for Deep Semi-Supervised Learning, Department of Electrical and Computer Engineering, University of Utah, 14.06.2016
    \bibitem{data-preprocessing-MRI} F. Agostini, S. Campese, R. Vianello, M. Pizzi, A. Cipriani, M. Zanetti, A post processing pipeline to prepare raw data for machine learning algorithms in cardiac magnetic resonance imaging, University of Padua, Włochy, 2022
    \bibitem{deep-learning-image-classification} Qing Lv, Suzhen Zhang, Yuechun Wang, Deep Learning Model of Image Classification Using Machine Learning, Shijiazhuang Posts and Telecommunications Technical College, Shijiazhuang 050021, Chiny, 2022
    \bibitem{image-augmentation-mobile-platform} Zihao Wang, Sen Yang, Mengji Shi, Kaiyu Qin, An Image Augmentation Method Based on Limited Samples for Object Tracking Based on Mobile Platform, School of Aeronautics and Astronautics, University of Electronic Science and Technology of China, Chengdu 611731, Chiny, 02.03.2022
    \bibitem{apple-image-feature-print} Dane za stroną internetową (dostęp 06.12.2023) \url{https://developer.apple.com/videos/play/wwdc2019/222}
    \bibitem{solar-flare-forecasting} Varad Deshmukh, Srinivas Baskar, Elizabeth Bradley, Thomas Berger, James D. Meiss, Machine Learning Approaches to Solar-Flare Forecasting: Is Complex Better?, Department of Computer Science, University of Colorado, Stany Zjednoczone, 18.02.2022
    \bibitem{smart-feature-selection} N. Yu Ilyasova, A. S. Shirokanev, R. A. Paringer, A. V. Kupriyanov, A. V. Zolotarev, A modified technique for smart textural feature selection to extract retinal regions of interest using image pre-processing, Samara National Research University, Rosja, 2018
    \bibitem{hybrid-neural-network-PCA} Keshun You, Guangqi Qiu, Yingkui Gu, Rolling Bearing Fault Diagnosis Using Hybrid Neural Network with Principal Component Analysis, School of Mechanical and Electrical Engineering, Jiangxi University of Science and Technology, Chiny, 17.11.2022
    \bibitem{feature-set-defect-prediction} Sana Gul, Rizwan Bin Faiz, Mohammad Aljaidi, Ghassan Samara, Ayoub Alsarhan, Ahmad al-Qerem, Impact Evaluation of Significant Feature Set in Cross Project for Defect Prediction through Hybrid Feature Selection in Multiclass, Faculty of Computing, Riphah International University Islamabad, Pakistan, 20.07.2023
    \bibitem{real-time-feature-subtraction} Nick Haber, Catalin Voss, Azar Fazel, Terry Winograd, Dennis P. Wall, A Practical Approach to Real-Time Neutral Feature Subtraction for Facial Expression Recognition, Stanford University, 2016
    \bibitem{burn-image-recognition-deep-learning} Xianjun Wu, Heming Chen, Xiaoli Wu, Shunjun Wu, Jinbo Huang, Burn Image Recognition of Medical Images Based on Deep Learning: From CNNs to Advanced Networks, Guangdong University of Petrochemical Technology, Guangdong, Chiny, 19.02.2021
    \bibitem{hybrid-linear-nonlinear-classifier} Weijie Chen, Charles E. Metz, Maryellen L. Giger, Karen Drukker, A Novel Hybrid Linear/Nonlinear Classifier for Two-Class Classification: Theory, Algorithm, and Applications, IEEE Transactions on Medical Imaging, 02.02.2010
    \bibitem{cnn-svm-comparison} Karthik B., Ramkumar G., Comparison of Feature Extraction Technique for Segmentation in Human Iris Recognition Under Uncontrolled Environment Using CNN Algorithm with SVM Classifier, ECS - The Electrochemical Society, 2022
    \bibitem{cnn-optimization} Xie Wenzhuo, Li Shiping, Xu Wei, Deng Haotian, Liao Weihan, Duan Xianbao, Wang Xuehua, Study on the CNN model optimization for household garbage classification based on machine learning, School of Materials Science and Engineering, Wuhan Institute of Technology, Wuhan, Chiny, 29.11.2022
    \bibitem{covid19-ct-classification} Chen Yao-Mei, Chen Yenming J., Ho Wen-Hsien, Tsai Jinn-Tsong, Classifying chest CT images as COVID‑19 positive/negative using a convolutional neural network ensemble model and uniform experimental design method, International Conference on Biomedical Engineering Innovation 2019, Kaohsiung, Tajwan, 11.2019
    \end{thebibliography}
\end{document}
