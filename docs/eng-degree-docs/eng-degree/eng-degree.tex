...
Ewaluacja obejmowała także analizę miar precyzji, czułości, specyficzności oraz miary F1, aby dostarczyć kompleksowej oceny skuteczności modelu.
...
Algorytm optymalizacyjny, tak jak SGD (\textit{eng. Stochastic Gradient Descent}), został wybrany do minimalizacji funkcji kosztu.
...
\begin{itemize}[label=--]
    ...
    \item bramka interfejsu aplikacji (eng. \textit{api gateway}) służąca za punkt dostępu do interfejsu, w której odbywa się komunikacja między mikroserwisami oraz autoryzacja i uwierzytelnianie użytkowników.
\end{itemize}
...
\forceindent Diagramy sekwencji przedstawiają istniejące w aplikacji serwerowej procesy, umożliwiające z niej użytkowanie.
...
Tenże link zawiera w sobie token o żywotności rzędu 24 godzin w postaci parametry kwerendy.
...
\subsection{Zużycie CPU: Flutter vs. aplikacje natywne}
\begin{figure}[htbp]
    \centering
    \textbf{\caption{Porównanie zużycia CPU: Flutter vs. aplikacje natywne na systemy iOS i Android}}
    \vspace{0.8em}
    \includegraphics[width=13.5cm, height={\imgth}, keepaspectratio]{./static/CPU-Usage-Comparison-Flutter-vs-Native-iOS-Android.png}
    \raggedright\scriptsize{\\Źródło: S. 27; Matilda Olsson, A Comparison of Performance and Looks Between Flutter and Native Applications}
\end{figure}

W badaniu porównano wydajność Fluttera z aplikacjami natywnymi na Androidzie i iOS pod kątem zużycia CPU. Stwierdzono, że aplikacje na iOS miały początkowo wyższe zużycie CPU, podczas gdy aplikacje na Androidzie miały podobne wyniki, z nieco lepszą wydajnością maksymalnego zużycia CPU dla Fluttera, ale niższą średnią dla aplikacji natywnych.
...
\subsection{Widoku aplikacji mobilnej}

\forceindent{Na rysunku 3.13 widoczny jest ekran startowy aplikacji mobilnej. Umożliwia on przejście do procesu rejestracji poprzez przycisk „Dołącz teraz” (eng. \textit{Join Now}) oraz do ekranu logowania przez kliknięcie w przycisk „Zaloguj się” (eng. \textit{Login}). }

\begin{figure}[H]
    \textbf{\caption{Ekran startowy aplikacji mobilnej}}
    \vspace{0.55em}
    \centering
    \includegraphics[width=8cm, keepaspectratio]{./static/mobile/start-page.jpg}\\
    \raggedright\scriptsize{Źródło: Opracowanie własne}
\end{figure}

% \begin{figure}[h]
%     \centering
%     \begin{subfigure}{\halflinewidth}%[position][height][inner pos]{width}
%         \textbf{\caption{Ekran startowy aplikacji mobilnej}}
%         \centering
%         \includegraphics[width=8cm, height={\imgth}, keepaspectratio]{./static/mobile/start-page.jpg}\\
%         \raggedright\scriptsize{Źródło: Opracowanie własne}
%     \end{subfigure}
%     \begin{subfigure}{\halflinewidth}
%         \textbf{\caption{Widok rejestracji}}
%         \centering
%         \includegraphics[width=8cm, height={\imgth}, keepaspectratio]{./static/mobile/register.jpg}\\
%         \raggedright\scriptsize{Źródło: Opracowanie własne}
%     \end{subfigure}
% \end{figure}

\forceindent{Rysunek 3.14 przedstawia widok rejestracji, dzięki któremu użytkownik może założyć konto, podając nazwę użytkownika, adres e-mail oraz hasło. W celu dodatkowej walidacji, hasło należy wprowadzić dwa razy. Po prawidłowym wpisaniu danych, należy kliknąć przycisk „Zarejestruj się” (eng. \textit{Register}). Z ekranu rejestracji istnieje możliwość łatwego przejścia do ekranu logowania poprzez kliknięcie w napis „Zaloguj się” (eng. \textit{Login}).}
...
\forceindent{Aby zalogować się do aplikacji, użytkownik musi podać swój adres email oraz hasło, co widoczne jest na rysunku 3.15. Proces logowania zatwierdza się, klikając przycisk „Zaloguj się” (eng. \textit{Login}). Z ekranu logowania istnieje również możliwość przejścia do panelu odzyskiwania hasła poprzez kliknięcie w napis „Zapomniałeś hasła?” (eng. \textit{Forgot password?}) oraz do ekranu rejestracji przez kliknięcie w „Zarejestruj się” (eng. \textit{Register}).}
...
\forceindent{Rysunek 3.16 przedstawia panel odzyskiwania hasła, który to wykorzystywane jest przez użytkowników w celu odzyskania hasła. Użytkownik powinien podać adres email powiązany z kontem, a następnie zatwierdzić operację za pomocą przycisku „Resetuj Hasło” (eng. \textit{Reset Password}). Po prawidłowym wykonaniu tej operacji, na podany adres email zostanie wysłana wiadomość z linkiem umożliwiającym odzyskanie hasła.}
...
\newpage
\forceindent{Strona główna (rysunek 3.17) prezentuje mapę z oznaczonymi lokalizacjami śmietników. Każdy typ śmietnika jest reprezentowany przez unikalny kolor i ikonę. Kliknięcie na tag śmietnika wyświetla dodatkowe informacje o danym punkcie. Na górze strony znajduje się pole tekstowe umożliwiające wyszukiwanie określonych lokalizacji. Po wpisaniu początku nazwy miasta, automatycznie pojawia się lista sugerowanych pasujących miejsc. Użytkownik może wybrać interesującą go lokalizację z tej listy. Na dole strony umieszczono menu nawigacyjne, dzięki któremu można łatwo poruszać się po aplikacji.}
...
\forceindent{Rysunek 3.18 przedstawia panel dodawnia punktów na mapie. Celem dodania nowego punktu na mapie, użytkownik powinien wybrać ikonę plusa w menu. Po otwarciu panelu, pierwszym krokiem jest dodanie zdjęcia. Zastosowany model uczenia maszynowego automatycznie rozpozna typ śmietnika. Jeśli rozpoznanie jest błędne, użytkownik ma możliwość ręcznej zmiany typu. Następnie, po weryfikacji typu śmietnika i lokalizacji, operację dodania punktu zatwierdza się przyciskiem „Zapisz” (eng. \textit{Save}).}
...
\forceindent{Panel użytkownika (rysunek 3.19) umożliwia edycję nazwy użytkownika oraz adresu email. Proces edycji rozpoczyna się od kliknięcia w przycisk „Edytuj” (eng. \textit{Edit}), co przekierowuje do odpowiedniego panelu edycyjnego. Funkcja „Ustawienia dostępu” (eng. \textit{Access settings}) pozwala na przeniesienie do sekcji ustawień, gdzie użytkownik może konfigurować uprawnienia aplikacji. Za pomocą suwaka trybu ciemnego (eng. \textit{dark mode}) istnieje możliwość zmiany wyglądu aplikacji z jasnego na ciemny i odwrotnie. Opcja „Wyloguj się” (eng. \textit{Sign out}) umożliwia bezpieczne wylogowanie się z konta.}
...
\forceindent{Panele edycji nazwy użytkownika i adresu email widoczne są odpowiednio na rysunkach 3.20 oraz 3.21. Celem edycji nazwy użytkownika, należy wpisać nową nazwę, a następnie zatwierdzić zmianę, klikając przycisk „Zapisz” (eng. \textit{Save}). Proces zmiany adresu email przebiega w identyczny sposób: użytkownik wpisuje nowy adres email i zatwierdza zmianę za pomocą tego samego przycisku „Zapisz” (eng. \textit{Save}).}
...
\begin{figure}[H]
    ...
    \centering
    \vspace{0.55em}
    \includegraphics[width=8cm, keepaspectratio]{./static/mobile/update-password.jpg}\\
    \raggedright\scriptsize{Źródło: Opracowanie własne}
\end{figure}
...
\forceindent W naszym projekcie wykorzystaliśmy wstępnie wytrenowaną konwolucyjną sieć neuronową (\textit{eng. Convolutional neural network - CNN}) z ekstraktorem funkcji Image Feature Print V2 w CreateML, usprawniając proces analizy danych obrazu dla naszego konkretnego zbioru danych.
...
\forceindent Ocena ta odzwierciedla wspólne wysiłki i osiągnięcia naszego zespołu projektowego w opracowaniu innowacyjnego rozwiązania do rozpoznawania obrazów odnoszących się do pojemników na odpady, podzielonych na kategorie według rodzaju odpadów. Projekt ten, prowadzony przez cały 2023 rok, zintegrował różne dyscypliny technologiczne, demonstrując zaangażowanie naszego zespołu w wykorzystanie technologii na rzecz zrównoważonego rozwoju środowiska.

\subsection{Zespół projektowy}

\forceindent Jakub Barczewski -- Pełnił funkcję lidera zespołu, koncentrował się na rozwoju aplikacji serwerowej i przygotowywaniu dokumentacji inżynierskiej przy użyciu środowiska LaTeX. Wykazał się umiarkowanym przywództwem w koordynowaniu działań projektowych i dokładał starań w kierunku zapewniania zgodności ze standardami akademickimi.

Marek Gerszendorf -- Zaangażowany w rozwój aplikacji mobilnej w języku Swift. Jego doświadczenie w dziedzinie UI/UX odegrało kluczową rolę w stworzeniu przyjaznego dla użytkownika interfejsu do kategoryzacji obrazów w czasie rzeczywistym. Niestrudzenie pracował nad rozwojem aplikacji mobilnej, aby zapewnić jej wysoką jakość.

Kacper Bylicki -- Specjalizował się w architekturze chmury i algorytmach uczenia maszynowego. Odegrał kluczową rolę we wdrażaniu solidnych rozwiązań do rozpoznawania obrazów i przetwarzania danych. Jest również twórcą prezentacji audio-wizualnej aplikacji końcowej.

\subsection{Wyzwania i rozwiązania}

\forceindent W trakcie trwania pracy projektowej, zespół napotkał problemy związane z komunikacją i zarządzaniem terminami. Problemy owe były okresowe i wpływały na dotrzymywanie terminów. Przyjęliśmy bardziej ustrukturyzowane harmonogramy spotkań i wykorzystaliśmy narzędzie Jira do współpracy oraz zarządzaniem zadaniami, co doprowadziło do poprawy przepływu informacji i śledzenia zadań.

Następnym wyzwaniem okazały się początkowe trudności z formatowaniem dokumentacji w środowisku LaTeX, co stanowiło zagrożenie dla spójności naszej dokumentacji. Dzięki zbiorowej nauce i wzajemnemu wsparciu pokonaliśmy te przeszkody, co zaowocowało dobrze zorganizowanym i akademicko rygorystycznym dokumentem końcowym, którego formatowanie jest bezwzględnie spójne.

\subsection{Pozytywne wyniki}

\forceindent Pomimo wyzwań, odporność i zdolność adaptacji zespołu doprowadziły do kilku pozytywnych rezultatów:

\begin{itemize}[label=--]
    \item opracowanie innowacyjnego rozwiązania -- wspólne wysiłki zespołu projektowego zaowocowały innowacyjnym rozwiązaniem, które skutecznie kategoryzuje kosze na śmieci za pomocą rozpoznawania obrazu, przyczyniając się do ochrony środowiska;
    \item interdyscyplinarna współpraca -- projekt był świadectwem siły interdyscyplinarnej współpracy, łączącej rozwój serwerów, przetwarzanie w chmurze, uczenie maszynowe i tworzenie aplikacji mobilnych;
    \item doskonalenie umiejętności -- członkowie zespołu znacznie poprawili swoje umiejętności techniczne i współpracy, kładąc solidne podstawy pod przyszłe przedsięwzięcia zawodowe.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Dokumentacja ścieżek żądań aplikacji serwerowej;
\end{enumerate}
